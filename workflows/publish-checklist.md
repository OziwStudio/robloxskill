...
- GamePass name, ID, price, and benefit.
- DevProduct name, ID, price, benefit, and repeat behavior.
- Premium benefits.
- Any ad integrations.

### GamePass Review

- Is the benefit obvious from the name and description?
- Does the player know exactly what they get before purchase?
- Is the value strong enough for the price?
- Does it provide lasting value, not a disguised consumable?

### DevProduct Review

- Does the product solve a real player need?
- Is the benefit immediate and satisfying?
- Does buying feel rewarding instead of annoying?
- Are larger bundles meaningfully better value?

### ProcessReceipt Review

- `ProcessReceipt` is server-side only.
- Every granted item is saved before acknowledgment.
- Unknown `ProductId` values are rejected.
- Receipt IDs are tracked to prevent duplicate grants.
- Only one `ProcessReceipt` assignment exists.

### Missing Opportunities

- Simulator: auto-collect, multiplier, storage, exclusive areas, boosts, instant rebirth.
- Tycoon: speed boost, extra slot, materials, income multiplier, unlock skip.
...
- Compare prices to genre norms.
- Flag products above or below market value.
- Check whether price matches value and game scale.
- Adjust only when the value story is clear.

### Premium Check

- Premium detection works.
- Premium rewards are meaningful but not game-breaking.
- Premium prompt appears at a sensible moment.
- Premium membership changes are handled during session.

### Ethical Review

- No pay-to-win advantage in competitive play.
- No artificial friction created to sell relief.
- No hidden odds in random rewards.
- No pressure tactics that block rational choice.
- No core gameplay locked behind payment.
